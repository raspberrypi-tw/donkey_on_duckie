...
class PWMSteering:
    ...
    def run(self, angle):
        ...
        print("===PWMSteerint===")
        print("left_pulse, right_pulse", self.left_pulse, self.right_pulse)
        print("pulse", pulse)

        half_pulse = (self.left_pulse + self.right_pulse)/2
        if (pulse > half_pulse):
            dc = (pulse - half_pulse)/(self.right_pulse - half_pulse)
        else:
            dc = (pulse - half_pulse)/(half_pulse - self.left_pulse)

        print("pulse", dc)
        self.controller.set_pulse(dc)
...
class PWMThrottle:
    ...
    def run(self, throttle):
        ...
        self.controller.set_speed(throttle)
...
class Adafruit_DCMotor_Hat:
    ...
    def set_speed(self, speed):
        if speed > 0:
            self.mhM1.run(self.BACKWARD)
            self.mhM2.run(self.FORWARD)
        else:
            self.mhM1.run(self.FORWARD)
            self.mhM2.run(self.BACKWARD)

        self.speed = int(speed * 255)

        try:
            print("pulse.speed", speed)

            if self.pulse > 0:
                speed = abs(int(self.speed*(1-self.pulse)))
                self.mhM2.setSpeed(speed)
                print("================")
                print("self.pulse: ", self.pulse)
                print("M2 Speed: ", speed)
                print("================")
            elif self.pulse < 0:
                speed = abs(int(self.speed*(1+self.pulse)))
                ...
                print("================")
                print("self.pulse: ", self.pulse)
                print("M1 Speed: ", speed)
                print("================")
            else:
                print("DC_MOTOR_SPEED", self.speed)
                self.mhM1.setSpeed(abs(self.speed))
                self.mhM2.setSpeed(abs(self.speed))
        except OSError as err:
            self.mhM1.run(self.RELEASE)
            self.mhM2.run(self.RELEASE)
            print("Unexpected issue setting PWM (check wires to motor board): {0}".format(err))
